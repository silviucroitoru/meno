...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: updateError } = await supabase.rpc("merge_ir_submission_response", {
      p_submission_id: SubmissionID,
      p_key: DataPointName,
      p_value: responseToStore,
    });

    if (updateError?.code === "PGRST202" || updateError?.message?.includes("merge_ir_submission_response")) {
