-- IR admin reporting: treat Phone (the final questionnaire step) as the
-- completion signal instead of Email. Email is collected one step before Phone,
-- so submissions with an email but no phone are dropouts that should not count
-- as completed nor appear in the admin list.
--
-- Scope: IR admin reporting functions only. No changes to questionnaire flow,
-- completion triggers, generate-ir-report, or menopause/contraception logic.

-- Admin metrics: count completed by Phone presence.
...
BEGIN
  ...
  SELECT count(*)
    ...
    WHERE nullif(trim(coalesce(responses->>'FirstName', '')), '') IS NOT NULL
      ...
      AND nullif(trim(coalesce(responses->>'Phone', '')), '') IS NOT NULL;
...
-- Admin list: only show submissions that completed the phone step.
...
BEGIN
  ...
  WITH base AS (
    ...
    WHERE nullif(trim(coalesce(s.responses->>'Email', '')), '') IS NOT NULL
      ...
      AND nullif(trim(coalesce(s.responses->>'Phone', '')), '') IS NOT NULL
  ),
  filtered AS (
    SELECT * FROM base b
...
-- Daily completed submissions: count by Phone presence.
...
-- Marketing range: count successful (completed) submissions by Phone presence.
